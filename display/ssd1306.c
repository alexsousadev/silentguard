...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  else if (c >= 'a' && c <= 'z') // Para letra minúsculas
  {
    index = (c - 'a' + 37) * 8;
  }
  else if (c == ':')
...
void ssd1306_init_config_clean(ssd1306_t *ssd, uint SCL, uint SDA, i2c_inst_t *PORT, uint8_t address)
{
  ...
  // Inicializa estrutura do display
  ssd1306_init(ssd, WIDTH, HEIGHT, false, address, PORT);
  ssd1306_config(ssd);
...
// método adicionado para limpar a tela
void ssd1306_clear_screen(ssd1306_t *ssd)
{
  // Limpa o display preenchendo com 'false' (desligando todos os pixels)
  ssd1306_fill(ssd, false);
  ssd1306_send_data(ssd); // Envia os dados para o display, efetivamente limpando a tela
}
